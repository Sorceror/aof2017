...
(defn spreadsheet-checksum [input]
  (->>
    (to-rows input)
    (map #(min-max-diff %))
    (reduce +)))

(spreadsheet-checksum puzzle-input)
...
(defn dividable [val others]
  (filter some?
    (flatten
      (map (fn [other-val]
             (if (= 0 (mod val other-val))
                 (/ val other-val)))
           (filter
             #(not= val %)
             others)))))
...
(defn div-row [row]
  (first
    (filter not-empty
      (map #(dividable % row) row))))

(->>
  (to-rows puzzle-input)
  (map #(div-row %))
  (flatten)
  (reduce +))
